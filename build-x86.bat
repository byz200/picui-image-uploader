...
echo ==========================================================
echo   Picui 图床上传工具 - x86 (windows/386) 编译
echo ==========================================================
...
if exist "%USERPROFILE%\go\bin\rsrc.exe" (
    "%USERPROFILE%\go\bin\rsrc.exe" -ico build\windows\icon.ico -arch 386 -o icon_windows.syso
    echo 图标资源已生成
) else (
    echo [提示] rsrc 工具不可用，跳过图标资源生成（exe 仍可用，但资源管理器图标为默认）
)

echo [4/4] 编译 Go (GOOS=windows GOARCH=386)...
set GOOS=windows
set GOARCH=386
set CGO_ENABLED=0
if not exist build\bin mkdir build\bin
go mod tidy
go build -tags desktop,production -ldflags "-s -w -H windowsgui" -o "build\bin\PicuiUploader-x86.exe" .
if %errorlevel%==0 (
    echo.
